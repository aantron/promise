  ...
  test(
    ...
    () => {
      let instrumentedLoop = n => {
        ...
        let rec theLoop: int => Promise.Js.t(int, unit) =
          n =>
            if (n == 0) {
              let wordsAllocated = countAllocatedWords() - initialWords;
      ...
      Promise.Js.catch(doesNotLeakMemory(instrumentedLoop, 100), () =>
        assert(false)
      );
    },
  );
...
let raceLoopTests =
  Framework.suite(
    ...
    [
      ...
      raceTest(
        "race loop memory leak", (foreverPendingPromise, nextIteration) => {
      ...
      raceTest(
        "race loop memory leak, with already-resolved promises",
      ...
      raceTest(
        "race loop memory leak, with rejection",
        (foreverPendingPromise, nextIteration) => {
      ...
      raceTest(
        "race loop memory leak, with already-rejected promises",
        (foreverPendingPromise, nextIteration) => {
      ...
      /* This test is like the first, but it tests for the interaction of the fixes
         ...
         lists. */
      raceTest(
        "race loop memory leak with flatMap merging",
        (foreverPendingPromise, nextIteration) => {
        let (shortLivedPromise, resolveShortLivedPromise, _) =
          Promise.Js.pending();
...
let allLoopTests =
  Framework.suite(
    ...
    [
      ...
      raceTest(
        "all loop memory leak, with already-rejected promises",
        (foreverPendingPromise, nextIteration) => {
        let rejectedPromise = Promise.Js.rejected();
