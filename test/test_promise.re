...
let basicTests =
  Framework.suite(
    ...
    [
      ...
      test("flatMap", () =>
        Promise.resolved(1)->Promise.flatMap(n => Promise.resolved(n == 1))
      ),
      test("map", () => {
      ...
      test("callback order (resolved later)", () => {
        ...
        p->Promise.map(() => firstCallbackCalled := true) |> ignore;
        p
        ->Promise.map(() => secondCallbackCalledSecond := firstCallbackCalled^)
        |> ignore;
        resolve();
...
let remainsPending = (p, dummyValue) => {
  ...
  repeat(10, () =>
    ...
    ->Promise.flatMap(v1 =>
        ...
        ->Promise.map(v2 => v1 == dummyValue && v2 == dummyValue)
      )
  );
};
  ...
  Framework.suite(
    ...
    [
      ...
      test("all2", () => {
        ...
        let (p2, resolveP2) = Promise.pending();
        let result =
          Promise.all2(p1, p2)->Promise.map(((x, y)) => x == 42 && y == 43);
        resolveP1(42);
        resolveP2(43);
        result;
      }),
...
let resultTests =
  Framework.suite(
    ...
    [
      ...
      test(">|=, ok", () =>
        Promise.Operators.(
          (Promise.resolved(Ok(42)) >|= (n => n + 1))
          ->Promise.map(v => v == Ok(43))
        )
      ),
      test(">|=, error", () =>
        Promise.Operators.(
          (Promise.resolved(Error(42)) >|= (n => n + 1))
          ->Promise.map(v => v == Error(42))
        )
      ),
      test(">>=, ok", () =>
        Promise.Operators.(
          (Promise.resolved(Ok(42)) >>= (n => Promise.resolved(Ok(n + 1))))
          ->Promise.map(v => v == Ok(43))
        )
      ),
      test(">>=, error", () =>
